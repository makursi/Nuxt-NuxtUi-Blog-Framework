...
watch(
    ...
    (newTitle) => {
        if (newTitle && !postInput.value.slug) {
            postInput.value.slug = slugify(newTitle, {
                lower: true,
                strict: true,
            });
        }
...
const createPost = async (): Promise<void> => {
    try {
        // 验证表单
        if (
            !postInput.value.title.trim() ||
            !postInput.value.post_content.trim()
        ) {
            return;
        }
        ...
        const response = await $fetch(
            `${config.public.API_BASE_URL}/posts/createpost`,
            {
                //请求头携带token
                headers: {
            ...
            <main class="container mx-auto px-4 py-8 max-w-4xl">
                ...
                >
                    ...
                    <div class="p-6 space-y-6">
                        ...
                        <!-- 操作按钮 -->
                        <div
                            class="flex items-center justify-between pt-6 border-t border-gray-700"
                        >
                            <UButton
                                variant="secondary"
                            ...
                            >
                                <template #left>
                                    <span
                                        class="i-heroicons-arrow-left mr-2"
                                    ></span>
                                </template>
                                Cancel
                            </UButton>
                            ...
                            >
                                <template #left>
                                    <span
                                        class="i-heroicons-paper-airplane mr-2"
                                    ></span>
                                </template>
